; ModuleID = 'LLVMDialectModule'
source_filename = "LLVMDialectModule"
target triple = "aie2p"

@flash_index_7_5 = external global [2 x i32]
@flash_state_7_5 = external global [256 x bfloat]
@flash_index_6_5 = external global [2 x i32]
@flash_state_6_5 = external global [256 x bfloat]
@flash_index_5_5 = external global [2 x i32]
@flash_state_5_5 = external global [256 x bfloat]
@flash_index_4_5 = external global [2 x i32]
@flash_state_4_5 = external global [256 x bfloat]
@flash_index_3_5 = external global [2 x i32]
@flash_state_3_5 = external global [256 x bfloat]
@flash_index_2_5 = external global [2 x i32]
@flash_state_2_5 = external global [256 x bfloat]
@flash_index_1_5 = external global [2 x i32]
@flash_state_1_5 = external global [256 x bfloat]
@flash_index_0_5 = external global [2 x i32]
@flash_state_0_5 = external global [256 x bfloat]
@flash_index_7_3 = external global [2 x i32]
@flash_state_7_3 = external global [256 x bfloat]
@flash_index_6_3 = external global [2 x i32]
@flash_state_6_3 = external global [256 x bfloat]
@flash_index_5_3 = external global [2 x i32]
@flash_state_5_3 = external global [256 x bfloat]
@flash_index_4_3 = external global [2 x i32]
@flash_state_4_3 = external global [256 x bfloat]
@flash_index_3_3 = external global [2 x i32]
@flash_state_3_3 = external global [256 x bfloat]
@flash_index_2_3 = external global [2 x i32]
@flash_state_2_3 = external global [256 x bfloat]
@flash_index_1_3 = external global [2 x i32]
@flash_state_1_3 = external global [256 x bfloat]
@flash_index_0_3 = external global [2 x i32]
@flash_state_0_3 = external global [256 x bfloat]
@flash_index_7_4 = external global [2 x i32]
@flash_index_6_4 = external global [2 x i32]
@flash_index_5_4 = external global [2 x i32]
@flash_index_4_4 = external global [2 x i32]
@flash_index_3_4 = external global [2 x i32]
@flash_index_2_4 = external global [2 x i32]
@flash_index_1_4 = external global [2 x i32]
@flash_index_0_4 = external global [2 x i32]
@flash_scale_0_3_buff_1 = external global [256 x bfloat]
@flash_scale_0_3_buff_0 = external global [256 x bfloat]
@flash_scale_1_3_buff_1 = external global [256 x bfloat]
@flash_scale_1_3_buff_0 = external global [256 x bfloat]
@flash_scale_2_3_buff_1 = external global [256 x bfloat]
@flash_scale_2_3_buff_0 = external global [256 x bfloat]
@flash_scale_3_3_buff_1 = external global [256 x bfloat]
@flash_scale_3_3_buff_0 = external global [256 x bfloat]
@flash_scale_4_3_buff_1 = external global [256 x bfloat]
@flash_scale_4_3_buff_0 = external global [256 x bfloat]
@flash_scale_5_3_buff_1 = external global [256 x bfloat]
@flash_scale_5_3_buff_0 = external global [256 x bfloat]
@flash_scale_6_3_buff_1 = external global [256 x bfloat]
@flash_scale_6_3_buff_0 = external global [256 x bfloat]
@flash_scale_7_3_buff_1 = external global [256 x bfloat]
@flash_scale_7_3_buff_0 = external global [256 x bfloat]
@flash_scale_0_5_buff_1 = external global [256 x bfloat]
@flash_scale_0_5_buff_0 = external global [256 x bfloat]
@flash_scale_1_5_buff_1 = external global [256 x bfloat]
@flash_scale_1_5_buff_0 = external global [256 x bfloat]
@flash_scale_2_5_buff_1 = external global [256 x bfloat]
@flash_scale_2_5_buff_0 = external global [256 x bfloat]
@flash_scale_3_5_buff_1 = external global [256 x bfloat]
@flash_scale_3_5_buff_0 = external global [256 x bfloat]
@flash_scale_4_5_buff_1 = external global [256 x bfloat]
@flash_scale_4_5_buff_0 = external global [256 x bfloat]
@flash_scale_5_5_buff_1 = external global [256 x bfloat]
@flash_scale_5_5_buff_0 = external global [256 x bfloat]
@flash_scale_6_5_buff_1 = external global [256 x bfloat]
@flash_scale_6_5_buff_0 = external global [256 x bfloat]
@flash_scale_7_5_buff_1 = external global [256 x bfloat]
@flash_scale_7_5_buff_0 = external global [256 x bfloat]
@of_0_mem_0_cons_buff_3 = external global [64 x [64 x bfloat]]
@of_0_mem_0_cons_buff_2 = external global [64 x [64 x bfloat]]
@of_0_mem_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_0_mem_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_0_mem_1_cons_buff_3 = external global [64 x [64 x bfloat]]
@of_0_mem_1_cons_buff_2 = external global [64 x [64 x bfloat]]
@of_0_mem_1_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_0_mem_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_0_mem_2_cons_buff_3 = external global [64 x [64 x bfloat]]
@of_0_mem_2_cons_buff_2 = external global [64 x [64 x bfloat]]
@of_0_mem_2_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_0_mem_2_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_0_mem_3_cons_buff_3 = external global [64 x [64 x bfloat]]
@of_0_mem_3_cons_buff_2 = external global [64 x [64 x bfloat]]
@of_0_mem_3_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_0_mem_3_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_0_mem_4_cons_buff_3 = external global [64 x [64 x bfloat]]
@of_0_mem_4_cons_buff_2 = external global [64 x [64 x bfloat]]
@of_0_mem_4_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_0_mem_4_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_0_mem_5_cons_buff_3 = external global [64 x [64 x bfloat]]
@of_0_mem_5_cons_buff_2 = external global [64 x [64 x bfloat]]
@of_0_mem_5_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_0_mem_5_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_0_mem_6_cons_buff_3 = external global [64 x [64 x bfloat]]
@of_0_mem_6_cons_buff_2 = external global [64 x [64 x bfloat]]
@of_0_mem_6_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_0_mem_6_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_0_mem_7_cons_buff_3 = external global [64 x [64 x bfloat]]
@of_0_mem_7_cons_buff_2 = external global [64 x [64 x bfloat]]
@of_0_mem_7_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_0_mem_7_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_1_mem_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_1_mem_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_2_mem_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_2_mem_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_3_unicast_0_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_3_unicast_0_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_3_unicast_1_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_3_unicast_1_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_3_unicast_2_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_3_unicast_2_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_3_unicast_3_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_3_unicast_3_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_3_unicast_4_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_3_unicast_4_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_3_unicast_5_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_3_unicast_5_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_3_unicast_6_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_3_unicast_6_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_3_unicast_7_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_3_unicast_7_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_7_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_7_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_6_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_6_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_5_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_5_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_4_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_4_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_3_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_3_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_2_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_2_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_1_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_4_broadcast_0_0_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_7_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_7_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_6_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_6_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_5_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_5_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_4_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_4_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_3_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_3_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_2_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_2_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_1_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_5_broadcast_0_0_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_0_0_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_0_1_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_0_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_1_0_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_1_1_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_1_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_2_0_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_2_1_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_2_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_3_0_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_3_1_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_3_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_4_0_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_4_1_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_4_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_5_0_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_5_1_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_5_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_6_0_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_6_1_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_6_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_7_0_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_7_1_buff_0 = external global [64 x [64 x bfloat]]
@of_6_switch_fork_7_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_0_0_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_0_0_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_0_1_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_0_1_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_1_0_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_1_0_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_1_1_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_1_1_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_2_0_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_2_0_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_2_1_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_2_1_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_3_0_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_3_0_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_3_1_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_3_1_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_4_0_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_4_0_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_4_1_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_4_1_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_5_0_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_5_0_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_5_1_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_5_1_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_6_0_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_6_0_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_6_1_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_6_1_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_7_0_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_7_0_buff_0 = external global [64 x [64 x bfloat]]
@of_7_switch_join_7_1_buff_1 = external global [64 x [64 x bfloat]]
@of_7_switch_join_7_1_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_0_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_0_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_0_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_0_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_1_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_1_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_1_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_1_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_2_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_2_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_2_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_2_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_3_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_3_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_3_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_3_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_4_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_4_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_4_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_4_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_5_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_5_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_5_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_5_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_6_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_6_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_6_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_6_cons_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_7_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_7_buff_0 = external global [64 x [64 x bfloat]]
@of_8_unicast_7_cons_buff_1 = external global [64 x [64 x bfloat]]
@of_8_unicast_7_cons_buff_0 = external global [64 x [64 x bfloat]]

declare void @debug_i32(i32)

; Unknown intrinsic
declare void @llvm.aie2p.event(i32)

; Unknown intrinsic
declare void @llvm.aie2p.put.ms(i32, i32)

; Unknown intrinsic
declare { i32, i32 } @llvm.aie2p.get.ss()

; Unknown intrinsic
declare void @llvm.aie2p.mcd.write.vec(<16 x i32>, i32)

; Unknown intrinsic
declare <16 x i32> @llvm.aie2p.scd.read.vec(i32)

; Unknown intrinsic
declare void @llvm.aie2p.acquire(i32, i32)

; Unknown intrinsic
declare void @llvm.aie2p.release(i32, i32)

; Unknown intrinsic
declare void @llvm.aie2p.set.ctrl.reg(i32, i32)

declare void @op0_zero_bf16_64_64_64(ptr)

declare void @op0_matmul_bf16_bf16_64_64_64(ptr, ptr, ptr)

declare void @op0_rescale_O(ptr, ptr, i32, ptr)

declare void @op0_zero_bf16(ptr)

declare void @op0_matmul_PV(ptr, ptr, ptr, ptr, i32, i32, ptr)

declare void @op0_init_scale_buffer(ptr, i32)

declare void @op0_passThroughLine(ptr, ptr, i32)

declare void @op0_partial_softmax_mode(ptr, ptr, ptr, ptr, bfloat, i32, i32, i32, i32, i32)

define void @core_0_4() {
  br label %1

1:                                                ; preds = %120, %0
  %2 = phi i64 [ %121, %120 ], [ 0, %0 ]
  %3 = phi i32 [ %11, %120 ], [ 0, %0 ]
  %4 = phi i32 [ %12, %120 ], [ 0, %0 ]
  %5 = phi i32 [ %13, %120 ], [ 0, %0 ]
  %6 = phi i32 [ %14, %120 ], [ 0, %0 ]
  %7 = phi i32 [ %15, %120 ], [ 0, %0 ]
  %8 = icmp slt i64 %2, 4294967295
  br i1 %8, label %9, label %122

9:                                                ; preds = %118, %1
  %10 = phi i64 [ %119, %118 ], [ 0, %1 ]
  %11 = phi i32 [ %22, %118 ], [ %3, %1 ]
  %12 = phi i32 [ %23, %118 ], [ %4, %1 ]
  %13 = phi i32 [ %24, %118 ], [ %5, %1 ]
  %14 = phi i32 [ %25, %118 ], [ %6, %1 ]
  %15 = phi i32 [ %26, %118 ], [ %7, %1 ]
  %16 = icmp slt i64 %10, 2
  br i1 %16, label %17, label %120

17:                                               ; preds = %9
  call void @llvm.aie2p.acquire(i32 48, i32 -2)
  call void @op0_zero_bf16(ptr @of_8_unicast_0_buff_0)
  call void @op0_zero_bf16(ptr @of_8_unicast_0_buff_1)
  %18 = trunc i64 %10 to i32
  %19 = mul i32 %18, 16
  br label %20

20:                                               ; preds = %107, %17
  %21 = phi i64 [ %117, %107 ], [ 0, %17 ]
  %22 = phi i32 [ %116, %107 ], [ %11, %17 ]
  %23 = phi i32 [ %110, %107 ], [ %12, %17 ]
  %24 = phi i32 [ %113, %107 ], [ %13, %17 ]
  %25 = phi i32 [ %59, %107 ], [ %14, %17 ]
  %26 = phi i32 [ %60, %107 ], [ %15, %17 ]
  %27 = icmp slt i64 %21, 32
  br i1 %27, label %28, label %118

28:                                               ; preds = %20
  call void @llvm.aie2p.acquire(i32 51, i32 -1)
  %29 = sext i32 %22 to i64
  switch i64 %29, label %30 [
    i64 0, label %123
    i64 1, label %125
  ]

30:                                               ; preds = %123, %125, %28
  %31 = phi ptr [ %126, %125 ], [ %124, %123 ], [ @of_5_broadcast_0_0_0_cons_buff_0, %28 ]
  %32 = getelementptr [64 x [64 x bfloat]], ptr %31, i32 0, i32 0, i32 0
  br label %33

33:                                               ; preds = %30
  call void @llvm.aie2p.acquire(i32 1, i32 -1)
  call void @llvm.aie2p.acquire(i32 33, i32 -1)
  %34 = sext i32 %23 to i64
  switch i64 %34, label %35 [
    i64 0, label %127
    i64 1, label %129
  ]

35:                                               ; preds = %127, %129, %33
  %36 = phi ptr [ %130, %129 ], [ %128, %127 ], [ @of_7_switch_join_0_0_buff_0, %33 ]
  %37 = getelementptr [64 x [64 x bfloat]], ptr %36, i32 0, i32 0, i32 0
  %38 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } { ptr inttoptr (i64 3735928559 to ptr), ptr poison, i64 poison, [2 x i64] poison, [2 x i64] poison }, ptr %37, 1
  %39 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %38, i64 0, 2
  %40 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %39, i64 64, 3, 0
  %41 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %40, i64 64, 3, 1
  %42 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %41, i64 64, 4, 0
  %43 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %42, i64 1, 4, 1
  br label %44

44:                                               ; preds = %35
  %45 = sext i32 %24 to i64
  switch i64 %45, label %46 [
    i64 0, label %131
    i64 1, label %133
  ]

46:                                               ; preds = %131, %133, %44
  %47 = phi ptr [ %134, %133 ], [ %132, %131 ], [ @of_7_switch_join_0_1_buff_0, %44 ]
  %48 = getelementptr [64 x [64 x bfloat]], ptr %47, i32 0, i32 0, i32 0
  %49 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } { ptr inttoptr (i64 3735928559 to ptr), ptr poison, i64 poison, [2 x i64] poison, [2 x i64] poison }, ptr %48, 1
  %50 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %49, i64 0, 2
  %51 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %50, i64 64, 3, 0
  %52 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %51, i64 64, 3, 1
  %53 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %52, i64 64, 4, 0
  %54 = insertvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %53, i64 1, 4, 1
  br label %55

55:                                               ; preds = %46
  %56 = trunc i64 %21 to i32
  br label %57

57:                                               ; preds = %103, %55
  %58 = phi i64 [ %106, %103 ], [ 0, %55 ]
  %59 = phi i32 [ %104, %103 ], [ %25, %55 ]
  %60 = phi i32 [ %105, %103 ], [ %26, %55 ]
  %61 = icmp slt i64 %58, 2
  br i1 %61, label %62, label %107

62:                                               ; preds = %57
  switch i64 %58, label %63 [
    i64 0, label %135
    i64 1, label %137
  ]

63:                                               ; preds = %135, %137, %62
  %64 = phi { ptr, ptr, i64, [2 x i64], [2 x i64] } [ %138, %137 ], [ %136, %135 ], [ { ptr inttoptr (i64 3735928559 to ptr), ptr @of_8_unicast_0_buff_0, i64 0, [2 x i64] [i64 64, i64 64], [2 x i64] [i64 64, i64 1] }, %62 ]
  br label %65

65:                                               ; preds = %63
  switch i64 %58, label %66 [
    i64 1, label %139
  ]

66:                                               ; preds = %139, %65
  %67 = phi { ptr, ptr, i64, [2 x i64], [2 x i64] } [ %140, %139 ], [ %43, %65 ]
  %68 = trunc i64 %58 to i32
  %69 = mul i32 %68, 8
  %70 = add i32 %69, %19
  store i32 %56, ptr @flash_index_0_4
  store i32 %70, ptr getelementptr inbounds nuw (i8, ptr @flash_index_0_4, i64 4)
  switch i64 %58, label %71 [
    i64 0, label %71
    i64 1, label %87
  ]

71:                                               ; preds = %66, %66
  call void @llvm.aie2p.acquire(i32 3, i32 -1)
  %72 = sext i32 %59 to i64
  switch i64 %72, label %73 [
    i64 0, label %141
    i64 1, label %143
  ]

73:                                               ; preds = %141, %143, %71
  %74 = phi ptr [ %144, %143 ], [ %142, %141 ], [ @flash_scale_0_3_buff_0, %71 ]
  %75 = getelementptr [256 x bfloat], ptr %74, i32 0, i32 0
  br label %76

76:                                               ; preds = %73
  %77 = icmp ne i32 %56, 0
  %78 = zext i1 %77 to i32
  %79 = extractvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %67, 1
  %80 = extractvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %64, 1
  call void @op0_matmul_PV(ptr %79, ptr %32, ptr %80, ptr %75, i32 64, i32 %78, ptr @flash_index_0_4)
  %81 = icmp eq i32 %56, 31
  br i1 %81, label %82, label %83

82:                                               ; preds = %76
  call void @op0_rescale_O(ptr %80, ptr %75, i32 64, ptr @flash_index_0_4)
  br label %83

83:                                               ; preds = %82, %76
  call void @llvm.aie2p.release(i32 2, i32 1)
  %84 = add i32 %59, 1
  %85 = icmp sge i32 %84, 2
  %86 = select i1 %85, i32 0, i32 %84
  br label %103

87:                                               ; preds = %66
  call void @llvm.aie2p.acquire(i32 37, i32 -1)
  %88 = sext i32 %60 to i64
  switch i64 %88, label %89 [
    i64 0, label %145
    i64 1, label %147
  ]

89:                                               ; preds = %145, %147, %87
  %90 = phi ptr [ %148, %147 ], [ %146, %145 ], [ @flash_scale_0_5_buff_0, %87 ]
  %91 = getelementptr [256 x bfloat], ptr %90, i32 0, i32 0
  br label %92

92:                                               ; preds = %89
  %93 = icmp ne i32 %56, 0
  %94 = zext i1 %93 to i32
  %95 = extractvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %67, 1
  %96 = extractvalue { ptr, ptr, i64, [2 x i64], [2 x i64] } %64, 1
  call void @op0_matmul_PV(ptr %95, ptr %32, ptr %96, ptr %91, i32 64, i32 %94, ptr @flash_index_0_4)
  %97 = icmp eq i32 %56, 31
  br i1 %97, label %98, label %99

98:                                               ; preds = %92
  call void @op0_rescale_O(ptr %96, ptr %91, i32 64, ptr @flash_index_0_4)
  br label %99

99:                                               ; preds = %98, %92
  call void @llvm.aie2p.release(i32 36, i32 1)
  %100 = add i32 %60, 1
  %101 = icmp sge i32 %100, 2
  %102 = select i1 %101, i32 0, i32 %100
  br label %103

103:                                              ; preds = %83, %99
  %104 = phi i32 [ %59, %99 ], [ %86, %83 ]
  %105 = phi i32 [ %102, %99 ], [ %60, %83 ]
  %106 = add i64 %58, 1
  br label %57

107:                                              ; preds = %57
  call void @llvm.aie2p.release(i32 0, i32 1)
  %108 = add i32 %23, 1
  %109 = icmp sge i32 %108, 2
  %110 = select i1 %109, i32 0, i32 %108
  call void @llvm.aie2p.release(i32 32, i32 1)
  %111 = add i32 %24, 1
  %112 = icmp sge i32 %111, 2
  %113 = select i1 %112, i32 0, i32 %111
  call void @llvm.aie2p.release(i32 50, i32 1)
  %114 = add i32 %22, 1
  %115 = icmp sge i32 %114, 2
  %116 = select i1 %115, i32 0, i32 %114
  %117 = add i64 %21, 1
  br label %20

118:                                              ; preds = %20
  call void @llvm.aie2p.release(i32 49, i32 2)
  %119 = add i64 %10, 1
  br label %9

120:                                              ; preds = %9
  %121 = add i64 %2, 1
  br label %1

122:                                              ; preds = %1
  ret void

123:                                              ; preds = %28
  %124 = phi ptr [ @of_5_broadcast_0_0_0_cons_buff_0, %28 ]
  br label %30

125:                                              ; preds = %28
  %126 = phi ptr [ @of_5_broadcast_0_0_0_cons_buff_1, %28 ]
  br label %30

127:                                              ; preds = %33
  %128 = phi ptr [ @of_7_switch_join_0_0_buff_0, %33 ]
  br label %35

129:                                              ; preds = %33
  %130 = phi ptr [ @of_7_switch_join_0_0_buff_1, %33 ]
  br label %35

131:                                              ; preds = %44
  %132 = phi ptr [ @of_7_switch_join_0_1_buff_0, %44 ]
  br label %46

133:                                              ; preds = %44
  %134 = phi ptr [ @of_7_switch_join_0_1_buff_1, %44 ]
  br label %46

135:                                              ; preds = %62
  %136 = phi { ptr, ptr, i64, [2 x i64], [2 x i64] } [ { ptr inttoptr (i64 3735928559 to ptr), ptr @of_8_unicast_0_buff_0, i64 0, [2 x i64] [i64 64, i64 64], [2 x i64] [i64 64, i64 1] }, %62 ]
  br label %63

137:                                              ; preds = %62
  %138 = phi { ptr, ptr, i64, [2 x i64], [2 x i64] } [ { ptr inttoptr (i64 3735928559 to ptr), ptr @of_8_unicast_0_buff_1, i64 0, [2 x i64] [i64 64, i64 64], [2 x i64] [i64 64, i64 1] }, %62 ]
  br label %63

139:                                              ; preds = %65
  %140 = phi { ptr, ptr, i64, [2 x i64], [2 x i64] } [ %54, %65 ]
  br label %66

141:                                              ; preds = %71
  %142 = phi ptr [ @flash_scale_0_3_buff_0, %71 ]
  br label %73

143:                                              ; preds = %71
  %144 = phi ptr [ @flash_scale_0_3_buff_1, %71 ]
  br label %73

145:                                              ; preds = %87
  %146 = phi ptr [ @flash_scale_0_5_buff_0, %87 ]
  br label %89

147:                                              ; preds = %87
  %148 = phi ptr [ @flash_scale_0_5_buff_1, %87 ]
  br label %89
}

!llvm.module.flags = !{!0}

!0 = !{i32 2, !"Debug Info Version", i32 3}
